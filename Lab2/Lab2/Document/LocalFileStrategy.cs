...
namespace Lab2.Document
{
    public class LocalFileStrategy: IStorageStrategy
    {
        public async Task SaveDocument(DocumentData data, string fileName)
        {
            ...
            var saver = DocumentFormatFactory.GetSaver(format);
            await saver.Save(fileName, data);
            //string format = Path.GetExtension(fileName).ToLower().TrimStart('.');
        ...
        public async Task<DocumentData> LoadDocument(string fileName)
        {
            ...
            var loader = DocumentFormatFactory.GetLoader(format);
            return await loader.Load(fileName);
            //if (!File.Exists(fileName))
            //    throw new FileNotFoundException("File not found");
            ...
            //{
            //    case "txt":
            //        return await LoadTxtAsync(fileName);
            //    case "json":
            //        return await LoadJsonAsync(fileName);
            //    case "xml":
            //        return await LoadXmlAsync(fileName);
            //    default:
            //        throw new ArgumentException("Unsupported file format");
            //}
